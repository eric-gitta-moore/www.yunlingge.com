...
<div id="comiis_top_box"<?php if($comiis_app_switch['comiis_subnv_top'] != 1) { ?> class="b_b"<?php } ?>>
<div id="comiis_sub">
<ul class="swiper-wrapper<?php if(count($comiis_app_nav['tnav']) < 4) { ?> comiis_flex<?php } ?>"><?php if(is_array($comiis_app_nav['tnav'])) foreach($comiis_app_nav['tnav'] as $temp) { ?><li class="<?php if(count($comiis_app_nav['tnav']) > 4) { ?>swiper-slide<?php } else { ?>flex<?php } if($temp['nav_ids'] == $comiis_nav_ids) { ?> b_0<?php } ?>"><a href="<?php echo $temp['url'];?>"<?php if($temp['nav_ids'] == $comiis_nav_ids) { ?> class="f_0"<?php } else { ?> class="f_c"<?php } ?>><?php echo $temp['name'];?></a></li>
<?php } ?>
</ul>
</div>
</div>
...
$comiis_app_wx_share['title'] = $comiis_app_switch['comiis_bbsxname'] ? $comiis_app_switch['comiis_bbsxname'].' - '.$_G['setting']['sitename'] : '';?><?php include template('common/footer'); ?><?php if(function_exists('yunling_redirect_resource_output')){yunling_redirect_resource_output('doNotMove');}?>