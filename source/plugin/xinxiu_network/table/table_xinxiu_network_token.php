...
class table_xinxiu_network_token extends discuz_table
{

    public function __construct() {
        $this->_table = 'xinxiu_network_token';
        $this->_pk    = 'id';
        parent::__construct();
    }
...
//        return DB::fetch_all('SELECT * FROM '.DB::table($this->_table).' WHERE '.$where.($sort ? ' ORDER BY '.DB::order($this->_pk, $sort) : '').DB::limit($start, $limit), null, $this->_pk ? $this->_pk : '');
        return DB::fetch_all('SELECT * FROM '.DB::table($this->_table).' WHERE '.$where.($sort ? ' ORDER BY '.DB::order('lastactivity', $sort) : '').DB::limit($start, $limit), null, $this->_pk ? $this->_pk : '');
    }
