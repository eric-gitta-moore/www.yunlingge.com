 ...
 * @link
 */
class lib_admin_thread
{
    protected static $table = 'plugin_thread';
    ...
    {
        lib_base::header(lib_base::admin_url('thread_moderate_list'));
    }
    
    public static function thread_moderate_list()
    {
        global $_G;
        ...
        $submit = lib_base::admin_url('thread_moderate_list');
        $submit .= '&status='.$status.'&limit='.$limit.'&modfid='.$modfid.'&dateline='.$dateline.'&author='.$escape['author'].'&subject='.$escape['title'];
        ...
        if(! empty($moderates)) {
            $modcount = C::t('forum_thread')->count_by_tid_fid(array_keys($moderates), $fids, $isgroup, $escape['author'], $escape['title']);
        }
        if($modcount != count($moderates) && ! $srcdate && ! $fids && ! $escape['author'] && ! $escape['title']) {
            lib_base::table(self::$table)->moderateswipe('tid', array_keys($moderates));
        }
        
        $threadlist = array();
        if($modcount) {
            $threadlist = C::t('forum_thread')->fetch_all_by_tid_fid(array_keys($moderates), $fids, $isgroup, $escape['author'], $escape['title'], $offset, $limit);
        ...
        foreach($threadlist as $key => & $thread)
        {
            ...
            $thread['message'] = str_replace(array('onmouseover','onload','onclick'), '', $thread['message']);
            $thread['censorwords'] = lib_base::table(self::$table)->censor($thread['subject'], $thread['message']);
            $thread['modthreadkey'] = modauthkey($thread['tid']);
            $thread['pic'] = array('title'=>lib_base::lang('pic_view'),'id'=>$thread['tid'],'start'=>0,'data'=>array());
            $thread['forum_name'] = $forums[$thread['fid']]['name'];
            ...
            if($thread['attachment'])
            {
                ...
                foreach(C::t('forum_attachment_n')->fetch_all_by_id('tid:'.$thread['tid'], 'tid', $thread['tid']) as $attach)
                {
                    ...
                    $attach['url'] = $attach['isimage'] ? '<img onclick=\'pic_show('.$thread['tid'].', '.$i.')\' src="'.$url.'" title="'.$image_title.'">' : $image_title.'<br><a href="'.$url.'" target="_blank">'.$attach['filename'].'</a>';
                    
                    $thread['message'] .= '<div class="layui-col-sm4 layui-col-md3 layui-col-lg2"><div class="layui-card attach-img">'.$attach['url'].'</div></div>';
                    
                    if($attach['isimage']){
                        $thread['pic']['data'][] = array(
                            'alt'=>$attach['filename'],
                            'pid'=>$attach['aid'],
                            'src'=>'/'.$url,
                            //'thumb'=>'/'.$url,
        ...
        $thread_dateline_select = lib_func::select_option(lib_base::table(self::$table)->thread_dateline, $dateline);
        
        $forum_select = '<option value="0"'.($modfid==0?'selected':'').'>'.lib_base::lang('thread_all').'</option><option value="-1"'.($modfid==-1?'selected':'').'>'.lib_base::lang('group_only').'</option>';
        ...
        $lang_subject = lib_base::lang('subject');
        $lang_moderate_range = lib_base::lang('moderate_range');
        $lang_search = lib_base::lang('search');
        $lang_thread_moderate_nodata = lib_base::lang('thread_moderate_nodata');
...
<a href="{$thread_moderate_url}" style="color:#009688;">{$lang_thread_moderate}</a>
...
<span class="layui-laypage-limits" style="margin:0;"><input name="subject" type="text" placeholder="{$lang_subject}" style="width:100px;margin:0;"value="{$subject}"}></span>
...
<span class="layui-laypage-limits" style="margin:0;"><select name="dateline" lay-ignore>{$thread_dateline_select}</select></span>  
</div>
<button class="layui-btn layui-btn-sm" onclick="$('body').find('form').submit();" style="margin-top:-5px;">{$lang_search}</button>
</div>
EOT;
        if(! $threadlist){
            $content .= '<div class="layui-card"><div class="layui-card-body" style="text-align:center;">'.$lang_thread_moderate_nodata.'</div></div>';
        }
        ...
        foreach($threadlist as $value)
        {
            ...
            $source_type = $value['isgroup']?'<a href="forum.php?mod=group&fid='.$value['fid'].'" target="_blank">'.$lang_group.'</a>':'<a href="forum.php?mod=forumdisplay&fid='.$value['fid'].'" target="_blank">'.$lang_forum.'-'.$value['forum_name'].'</a>';
            
            $content .= <<<EOT
            
<div class="layui-card">
    <div class="layui-card-header"><h3 onclick="show_content($(this))"><a href="javascript:;">{$value['subject']}</a></h3></div>
    <div class="layui-card-body">
       ...
       {$censorwords}&nbsp;
       <a href="javascript:detail_show('{$lang_thread_detail}', 'forum.php?mod=viewthread&tid={$value['tid']}&modthreadkey={$value['modthreadkey']}');">{$lang_look}</a>
    ...
    <input onclick="bg_show($(this), 'validate')" class="radio" type="radio" name="moderate[{$value['tid']}]" value="validate" lay-ignore> <label>{$lang_validate}</label>&nbsp;&nbsp;
    <input onclick="bg_show($(this), 'delete')" class="radio" type="radio" name="moderate[{$value['tid']}]" value="delete" lay-ignore> <label>{$lang_delete}</label>&nbsp;&nbsp;
    <input onclick="bg_show($(this), 'ignore')" class="radio" type="radio" name="moderate[{$value['tid']}]" value="ignore" lay-ignore> <label>{$lang_ignore}</label>
    &nbsp;&nbsp;&nbsp;    
    <a href="javascript:;" onclick="verify($(this), 'validate', {$value['tid']})">{$lang_validate}</a> -
    <a href="javascript:;" onclick="verify($(this), 'delete', {$value['tid']})">{$lang_delete}</a> -
    <a href="javascript:;" onclick="verify($(this), 'ignore', {$value['tid']})">{$lang_ignore}</a>
    &nbsp;&nbsp;&nbsp;   
    {$lang_op_reason}
    <input type="text" name="pm_{$value['tid']}" id="pm_{$value['tid']}" class="op_input" style="width:100px;">
    <select onchange="$('#pm_{$value['tid']}').val(this.value)" class="op_select" lay-ignore>{$modreasonoptions}</select>
    </div>
</div>
<script>photos[{$value['tid']}] = {$pic_json};</script>
EOT;
        }
...
EOT;
        ...
        $moderate_thread_url = lib_base::admin_url('moderate_thread');
...
function verify(e, type, tid){
    var url = '{$moderate_thread_url}&moderate['+tid+']='+type;
    $.ajax({
		dataType: 'json',
		type    : 'post',
		url     : url,
...
EOT;
    ...
    public static function moderate_thread()
    {
        $moderate = $_GET['moderate'];
        
        $result = lib_base::table(self::$table)->moderate_thread($moderate);

        $msg = sprintf(lib_base::lang('thread_moderate_success'), $result['validates'], $result['ignores'], $result['recycles'], $result['deletes']);
