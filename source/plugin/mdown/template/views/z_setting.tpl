...
<head>
  ...
  <script>
    function showErrtips() {
		...
		for (var fun in v.checkfuncs) {
			if (!v.checkfuncs[fun]) {
            	ls.push('<li class="err">找不到 '+fun+' 函数，请联系网站运维技术员安装相应扩展。</li>');
			}
		}
	...
	<tr>
      ...
      <td>
          <label><input name="strategy" type="radio" value="1"> 随机头像
              <span class="tips2">（从<a href="admin.php?action=plugins&operation=config&identifier=extavatar&pmod=z_avatar">头像库</a>中随机选择一个头像分配给用户）</span>
          </label><br>
