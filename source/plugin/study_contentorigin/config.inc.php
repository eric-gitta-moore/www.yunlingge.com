...
/**
 *序列号：2020012608tTf4V5ekv4
 *应用版本：8737
 *发布时间：1574702178
 *站点URL：http://www.yun-ling.cn/	
 *客户端URL：http://www.yun-ling.cn/	
 *站点ID：60DF4F84-4325-8E2C-A68B-78184AFF8D9A	
 *QQID：87810FE7-AAAE-55ED-A938-30D35972DF05
 */
$pluginvars = array();
	...
	if($pluginvars) {
		...
		foreach($pluginvars as $var) {
			...
			if($var['type'] == 'number') {
				...
				$var['type'] = 'calendar';//站点ID：60DF4F84-4325-8E2C-A68B-78184AFF8D9A	
				...
				$var['variable'] = $var['value'] = '';
			} elseif(substr($var['type'], 0, 5) == 'group') {//发布时间：1574702178
				if($var['type'] == 'groups') {
					$var['description'] = ($var['description'] ? (isset($lang[$var['description']]) ? $lang[$var['description']] : $var['description'])."\n" : '').$lang['plugins_edit_vars_multiselect_comment']."\n".$var['comment'];
					$var['value'] = dunserialize($var['value']);
				...
				$var['variable'] = $var['value'] = '';//客户端URL：http://www.yun-ling.cn/
			} elseif($var['type'] == 'extcredit') {//应用版本：8737
				$var['type'] = '<select name="'.$var['variable'].'"><option value="">'.cplang('plugins_empty').'</option>';
				foreach($_G['setting']['extcredits'] as $id => $credit) {
					$var['type'] .= '<option value="'.$id.'"'.($var['value'] == $id ? ' selected' : '').'>'.$credit['title'].'</option>';
				}
				$var['type'] .= '</select>';
				$var['variable'] = $var['value'] = '';//站点URL：http://www.yun-ling.cn/
			}
