...
foreach($config as $k => $c){
	...
	showtablerow('', array('width="20"', 'width="100"','class="td28"','class="td28"','class="td28"'), 
			array('',
			'<img src="source/plugin/dc_pay/api/'.$k.'/'.$c['logo'].'" />',
			$c['title'],
			$c['des'],
			$c['author'],
			$c['version'],
...
foreach($payarr as $k => $c){
	showtablerow('', array('width="20"', 'width="100"','class="td28"','class="td28"', 'class="td28"'), array('','<img src="source/plugin/dc_pay/api/'.$k.'/'.$c['logo'].'" />',$c['title'],$c['des'],$c['author'],$c['version'],'<a href="'.ADMINSCRIPT.'?action=plugins&operation=config&do='.$pluginid.'&identifier=dc_pay&pmod=paylist&act=install&f='.$k.'">'.$_lang['install'].'</a>'));
}
showtablefooter();
if($payerror){
	writeconfig($config);
