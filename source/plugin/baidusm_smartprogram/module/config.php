...
function upsertConfig($params) {
    $adConfig = isset($params['adConfig']) ? htmlspecialchars($params['adConfig']) : "";
    if (empty($adConfig)) {
    ...
    $createTime = isset($params['createTime']) ? intval($params['createTime']) : time();
    ...
    if (empty($res)) {
        //插入数据
        $sqlInsert = "INSERT INTO ".DB::table('swan_app_config')." SET `ad_config`='".$data['ad_config']."' , `update_time`='".$data['update_time']."' , `create_time`='".$createTime."'";
