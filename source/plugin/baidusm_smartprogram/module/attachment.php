...
try {
    switch ($params['action']) {
        // 附件下载
        case "download":
            require DISCUZ_ROOT . "./source/plugin/baidusm_smartprogram/module/attachment_download.php";
            break;
        default:
            //初始化上传文件
            ...
            $file = $_FILES['file'];
            $arrResonse['data'] = uploadFile($file, $params, $_G);
    }
 ...
 * @date 06/06/2019
 */
function uploadFile($file, $params, $_G) {
    $arrRet = array(
        'attachment' => array(),
    );
    ...
    //文件存储路径
    $arrRet['attachment'][] = getAttachPrexUrl(ATTACHMENT, $_G['setting']['ftp']['on']). $dirTmp . '/' .$attach['attachment'];
