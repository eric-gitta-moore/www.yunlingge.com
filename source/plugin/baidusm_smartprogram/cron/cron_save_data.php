<?php
/**
 * @name: cron_save_data.php
 * @desc:
 * @author: (songshouming)
 * Time: 2019-10-23 15:22
 */

//cronname:百度小程序统计板块与用户的关系定时任务
//week:0
//day:0
//hour:0
//minute:31
    ...
    while ($page <= $totalPage) {
        ...
        foreach ($datas as $data) {
            ...
            if (!empty($data['viewperm'])) {
                ...
                $viewgroups = array_unique($viewgroups);
                $viewgroups_install = array();

                // 没有设定特定用户
                if (empty($usernames)) {
                    foreach ($viewgroups as $viewgroup) {
                        $viewgroup = intval($viewgroup);
                        if (!empty($viewgroup)) {
                            if (in_array($viewgroup, $viewgroups_install)) {
                                continue;
                            }
                            $viewgroups_install[] = $viewgroup;
...
function getThreadScore() {
    ...
    while ($page <= $totalPage) {
        ...
        $threads = DB::fetch_all("select tid, fid, dateline, lastpost, views, replies, digest, closed, heats, favtimes, sharetimes, displayorder from %t where dateline < {$time} order by tid desc ".
