...
class PhoneAuth {
    ...
    private function validate($scene = 0) {
        $token = $_REQUEST['vaptcha_token'];
        $knock = $_REQUEST['vaptcha_challenge'];
        return $this->vaptcha->validate($knock, $token, $scene);
    }
    ...
    // test method
    private function sendCodeMsgTest($phone, $token, $countrycode, $type = 'default') {
        if(strlen($phone) < 6){
            return $this->response(401, 'phone_rule_error',  'phone');
        }
        $countrycode = get_params('enable_inter') == '1' ? $countrycode : '86';
    ...
    public function login() {
        global $_G;
        if (get_params('login_captcha') != '0' && !$this->validate(1)) {
            return $this->response(401, 'validate_failure', 'vaptcha');
        }
    ...
    public function register() {
        ...
        if ($code['value'] != $_REQUEST['code'] ) {
            return $this->response(401, 'code_is_error', $code['value']);
        }
        global $_G;
    ...
    public function getsms() {
        ...
        return $this->sms->getSms($vid, $key);
        // return '';
    }

    public function getsmslast() {
        $smskey = trim($_REQUEST['smskey']);
        $smsid = trim($_REQUEST['smsid']);
        return $this->sms->getsmslast($smsid, $smskey);
    }
}
