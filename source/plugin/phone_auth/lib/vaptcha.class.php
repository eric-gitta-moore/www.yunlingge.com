...
class Vaptcha
{
    ...
    public function getChallenge($sceneId = 0)
    {
        ...
        if (!self::$isDown) {
            $knock = self::readContentFormGet($url);
    ...
    private function getIsDwon()
    {
        ...
        if($channel['offline_state'] == 1) {self::$isDown = true;return true;}
        return false;
    }
    ...
    private function downTimeCheck($callback, $v, $knock)
    {
        ...
        $imgs = Session::getValue($knock);
        Session::delete($knock);
        $address = md5($v.$imgs);
        $url = DOWNTIME_URL.$offline_key.'/'.$address;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        ...
        if($httpCode == 200) {
            ...
            Session::set($token, $this->getCurrentTime());
    ...
    private function downTimeValidate($token)
    {
        ...
        if (count($strs) < 2) {
            ...
            // $signature = $strs[1];
            $storageTIme = Session::getValue($token);
            $now = $this->getCurrentTime();
    ...
    private function getDownTimeCaptcha($callback = null)
    {
        ...
        $data = $this->getChannelData();
        $knock = md5($captcha . $$time . self::$publicKey);
        $ul = $this->getImgUrl();
