...
class plugin_apoyl_video
{
    ...
    public function discuzcode($value)
    {
        ...
        if (in_array($_G['fid'], unserialize($cache['openforums']))) {
            if ($cache['openmp4'])
    ...
    private function _callback_videoapoyl($match)
    {
        global $_G;
        $cache = $_G['cache']['plugin']['apoyl_video'];
        $width = $cache['mp4width'] ? $cache['mp4width'] : 600;
        $height = $cache['mp4height'] ? $cache['mp4height'] : 400;
...
class plugin_apoyl_video_portal extends plugin_apoyl_video{
    public function view_go_output($a){
        global $_G,$content;
        $cache = $_G['cache']['plugin']['apoyl_video'];
        include $this->_fileapoylv2('articlepcmp4apoyl');
