...
class table_rjy_log extends discuz_table
{
	...
	public function fetch_by_group($nums,$field='*') {
		$nums = intval($nums);
		return DB::fetch_all("SELECT $field FROM %t WHERE (status=1 or status=5) and listtype=2 order by paytime desc limit $nums", array($this->_table));
	}
