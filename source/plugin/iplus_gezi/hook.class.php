...
class plugin_iplus_gezi {
	...
	function _creat_gezi(){
		...
		if($_G['timestamp']-intval($lasttime)>$cache||$maxnum!=count($links)){//5分钟更新缓存 或者有增加行也更新
			...
			while($value=DB::fetch($query)){
				if($value){
					$style='style="';
					$value['title'] = dhtmlspecialchars(trim($value['title']));
					$value['title'] = cutstr($value['title'],$wzlen,'');
	...
	function _pageplay(){
		...
		if($page==2&&(CURSCRIPT=='forum')){//论坛首页
			return 1;
		}
	...
	function global_footerlink(){
		loadcache('plugin');
		global $_G;
		if($this->nav==4) return '<a href="home.php?mod=spacecp&ac=plugin&id=iplus_gezi:adlist">'.$this->name.'</a>';	//<span class="pipe">|</span>
	}
	
	function global_usernav_extra1(){
		loadcache('plugin');
		global $_G;
		if($this->nav==5) return '<li><a href="home.php?mod=spacecp&ac=plugin&id=iplus_gezi:adlist">'.$this->name.'</a></li>';	
		
	}	
}

class plugin_iplus_gezi_forum extends plugin_iplus_gezi {
