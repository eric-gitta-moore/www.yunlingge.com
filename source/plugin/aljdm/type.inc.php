...
else
{
	...
	if(is_array($order)) {
		foreach($order as $id=>$value) {
			...
			$bindlink_array = explode(':',$bindlink[$id]);
			if (count($bindlink_array) != 1)
            {
                $bindlink_id = $bindlink_array[0];
            }
			else
            {
                $bindlink_id = $bindlink[$id];
            }
            domain_create(trim($bindlink_id),$value,$name[$id]);
		}
	}
	...
	if(is_array($newcat)) {
		foreach($newcat as $key=>$name) {
            ...
            $newbindlink_array = explode(':',$newbindlink[$key]);
            if (count($newbindlink_array) != 1)
            {
                $newbindlink_id = $newbindlink_array[0];
            }
            else
            {
                $newbindlink_id = $newbindlink[$key];
            }
            domain_create(trim($newbindlink_id),$newcatorder[$key],$name);
		}
	}
	...
	$domain_file = DISCUZ_ROOT.'./data/sysdata/cache_domain.php';
	if (is_file($domain_file) && !empty($aljdm))
    {
        $domain = [];
        @include $domain_file;
        if (array_key_exists('list',$domain) && !empty($domain))
        {
            foreach ($aljdm as $key => $value) {
                $plugin_id = $value['displayorder'].'.'.$value['subject'];
                if (array_key_exists($plugin_id,$domain['list']))
                {
                    $domain['list'][$plugin_id]['id'] = $value['bindlink'];
                }

            }

            writetocache('domain', getcachevars(array('domain' => $domain)));
        }
    }
