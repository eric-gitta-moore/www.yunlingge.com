...
class plugin_nimba_romotepic_forum extends plugin_nimba_romotepic
{
    ...
    {
        global $_G;
        $down = intval(getgpc('romotepic'));
        $no_down = intval(getgpc('no_down'));
        $siteurl = $this->siteurl ? $_G['siteurl'] : $this->siteurl;

        if ($no_down)
        {
            $this->timeDirPath = dgmdate(TIMESTAMP, 'Ym/d/');
        }

        if ($down && $this->open && in_array($_G['groupid'], $this->group) && (in_array($_G['fid'], $this->forum) || dempty($this->forum))) {
            if (preg_match_all("/\[img[^\]]*\](.*)\[\/img\]/isU", $_GET['message'], $result)) {//匹配[img]
                ...
                foreach ($result[1] as $key => $src) {
                    ...
                    // var_dump($src);
                    // var_dump(pathinfo($src));
                    // exit();

                    if ($no_down)
                    {
                        $localattachment = $this->timeDirPath . pathinfo($src)['filename'] . '.' . $ext;
                    }
                    else
                    {
...
//                    exit();
                    ...
                    if ($attachsavedsize) {
                        ...
                        if ($watermarkstatus['forum'] && empty($_G['forum']['disablewatermark']) && !$no_down) {
