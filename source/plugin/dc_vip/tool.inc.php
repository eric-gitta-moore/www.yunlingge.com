...
$entrydir = DISCUZ_ROOT.'source/plugin/dc_vip/tool/';
$extends = array();
	...
	while($f = $d->read()) {
		if(preg_match('/^tool\_(\w+)?\.php$/', $f, $a)){
			if(!in_array($a[1],array('base'))){
				C::import('tool/'.$a[1],'plugin/dc_vip',false);
				$modstr = 'tool_'.$a[1];
...
$f=trim($_GET['f']);
if($f&&$extends[$f]){
	C::import('tool/'.$f,'plugin/dc_vip',false);
	$modstr = 'tool_'.$f;
	if (class_exists($modstr,false)){
		$mobj = new $modstr();
		if(submitcheck('submit',true)&&FORMHASH==$_GET['formhash']){
			...
			cpmsg(plang('setsucceed'), 'action=plugins&operation=config&do='.$pluginid.'&identifier=dc_vip&pmod=tool', 'succeed');
		}
...
foreach($extends as $k => $v){
	showtablerow('', array('','', 'width="20%"'), array($v['title'],$v['des'], '<a href="?action=plugins&operation=config&identifier=dc_vip&pmod=tool&f='.$k.'">'.plang('tool_action').'</a>'));
}
showtablefooter();
function plang($str) {
