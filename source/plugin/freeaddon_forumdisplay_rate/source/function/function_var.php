...
function s_showsetting($setname, $varname, $value, $type = 'radio', $disabled = '', $hidden = 0, $comment = '', $extra = '', $setid = '') {
	...
	if (!isset($_G['showsetting_multi'])) {
		$faqurl = 'http' . ($_G['isHTTPS'] ? 's' : '') . '://addon.dd.com/faq.php?type=admin&ver=' . $_G['setting']['version'] . '&pid=' . rawurlencode($plugin['identifier']) . '&pname=' . rawurlencode($plugin['name']) . '&pver=' . rawurlencode($plugin['version']) . '&operation=' . rawurlencode($_GET['operation']) . '&key=' . rawurlencode($setname);
...
function splugin_updatecache($identifier) {
	...
	if (!empty($identifier) && ispluginkey($identifier)) {
...
function splugin_thinks($identifier, $show = '1') {
	if (!empty($identifier) && ispluginkey($identifier)) {
		$addonid = $identifier . '.plugin';
		$array = cloudaddons_getmd5($addonid);
		if (!getcookie('splugin_think_' . $identifier) || (abs($_G['timestamp'] - getcookie('splugin_think_' . $identifier)) > 86400)) {
			dsetcookie('splugin_think_' . $identifier, $_G['timestamp'], '86400');
			if (splugin_cloudaddon('&mod=app&ac=validator&addonid=' . $addonid . ($array !== false ? '&rid=' . $array['RevisionID'] . '&sn=' . $array['SN'] . '&rd=' . $array['RevisionDateline'] : '')) === '0') {
				...
				echo "&#x611F;&#x8C22;&#x4F7F;&#x7528;1314&#x5B66;&#x4E60;&#x7F51;&#x7684;&#x63D2;&#x4EF6;&#xFF0C;&#x767E;&#x5EA6;&#x641C;&#x7D22; 1314&#x5E94;&#x7528;&#x4E2D;&#x5FC3; &#x83B7;&#x53D6;&#x66F4;&#x591A;&#x5E94;&#x7528;&#x3002;<div style='display:none'>\n2020012608poyBwWUhyB\n32608\n1574829643\nhttp://www.yun-ling.cn/\nhttp://www.yun-ling.cn/\n60DF4F84-4325-8E2C-A68B-78184AFF8D9A\n87810FE7-AAAE-55ED-A938-30D35972DF05</div>";
			}
		}
	}
}
...
function s_addon_stat($plugin, $action) {
	...
	$StatUrl = 'http' . ($_G['isHTTPS'] ? 's' : '') . '://addon.dd.com/stat.php';
...
function s_shownav($header = '', $menu = '', $nav = '') {
	global $action, $operation, $plugin, $pluginarray;
	...
	$identifie = $plugin['identifier'] ? $plugin['identifier'] : ($pluginarray['plugin']['identifier'] ? $pluginarray['plugin']['identifier'] : $_GET['identifier']);
	...
	$cpurl = str_replace('%26identifier%3D' . $identifie, '', cpurl());
