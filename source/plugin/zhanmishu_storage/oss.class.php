...
class plugin_zhanmishu_storage {
	function common(){
		global $_G;
		//$_G['setting']['ftp']['on'] = 1;
...
class plugin_zhanmishu_storage_home extends plugin_zhanmishu_storage {
	...
	// 	include template('zhanmishu_storage:blog_upload');
...
class plugin_zhanmishu_storage_forum extends plugin_zhanmishu_storage {
	...
	function post_upload_extend(){
		...
		$imgexts = str_replace(array(';', '*.'), array(', ', ''), $swfconfig['imageexts']['ext']);
		$allowpostimg = $_G['group']['allowpostimage'] && $imgexts;
		include template('zhanmishu_storage:upload');
	...
	function forumdisplay_fastpost_upload_extend(){
		...
		$imgexts = str_replace(array(';', '*.'), array(', ', ''), $swfconfig['imageexts']['ext']);
		$allowpostimg = $_G['group']['allowpostimage'] && $imgexts;
		include template('zhanmishu_storage:fast_upload');
	...
	function image(){
		...
		if($attach = C::t('forum_attachment_n')->fetch('aid:'.$daid, $daid, array(1, -1))) {
			...
			if($attach['remote']) {
				...
				// var_dump('/forum/'.$attach['attachment']);
				// var_dump($_G['setting']['ftp']['attachurl']);
				// echo $_G['setting']['ftp']['attachurl'].$signUrl;
				// die;
	...
	function viewthread_bottom_output() {
		...
		foreach($postlist as $k=>$post){
			foreach($post['attachments'] as $key=>$attach){
				...
				}else if($attach['remote']== '1' && $attach['isimage']){
					if ($config['iswater']) {
						$water = '/'.$config['water_field'];
					}
					$post['message'] = str_replace('/forum/'.$attach['attachment'].'"',$this->Oss_Signatureurl('/forum/'.$attach['attachment'].$water).'"',$post['message']);
				}
			}
