...
$server_url = 'action=plugins&operation=config&do=' . $pluginid . '&identifier=zhiwu55com_autoreply&pmod=user_vest&subpmod=regvest&formhash=' . FORMHASH;
if ($_GET['formhash'] == FORMHASH && empty($_GET['adduser']) == false && $_GET['adduser'] == 'yes') {
	...
	{
		cpmsg("zhiwu55com_autoreply:reg_err", '', "error");
	}
	...
	foreach ($regData as $regValue) {
		...
		DB::insert('zhiwu55comautoreply_reguser', $addUser);
	...
	cpmsg("zhiwu55com_autoreply:registered_successfully", $server_url, "succeed");
	...
	include template('zhiwu55com_autoreply:regvest');
