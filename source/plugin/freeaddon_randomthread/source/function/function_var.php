...
function s_showsetting($setname, $varname, $value, $type = 'radio', $disabled = '', $hidden = 0, $comment = '', $extra = '', $setid = '') {
	...
	if (!isset($_G['showsetting_multi'])) {
		$faqurl = 'http' . ($_G['isHTTPS'] ? 's' : '') . '://addon./faq.php?type=admin&ver=' . $_G['setting']['version'] . '&pid=' . rawurlencode($plugin['identifier']) . '&pname=' . rawurlencode($plugin['name']) . '&pver=' . rawurlencode($plugin['version']) . '&operation=' . rawurlencode($_GET['operation']) . '&key=' . rawurlencode($setname);
...
function s_addon_stat($plugin, $action) {
	...
	$StatUrl = 'http' . ($_G['isHTTPS'] ? 's' : '') . '://addon./stat.php';
