(function($) {
	$.fn.nbspSlider = function(options){
		var defaults = {
			...
			altjyFontColor:    '#fff',        // ALT区块内的字体颜色
		...
		return this.each(function() {  
			...
			$(obj).append(
				...
				+'<div id="altbox"><span class="title">'+$(obj).find("ul li span").eq(0).html()
				+'</span><br><span class="summary">'+$(obj).find("ul li div").eq(0).html()
				+'</span></div><div id="btnList"></div>'
			);
			...
			$("#altbox .summary",obj).css({
				'color':options.altjyFontColor
			});
			...
			$(".sliderBtn",obj).css("opacity",0.3).hover(function(){
				...
				options.effect == "horizontal" 
					? options.noEffect == 1 
						...
						: ((start_index == 0 || start_index == end_index) && options.starEndNoEff == 1 ) 
							? $(this).css("margin-left",extent) 
							: $(this).animate({marginLeft:extent},options.speeds) 
			...
			function AltFun(index){
				...
				$(obj).find("#altbox").html('<span class="title">'+$(obj).find("ul li span").eq(index).html()	+'</span><br><span class="summary">'+$(obj).find("ul li div").eq(index).html()+'</span>');	
				$("#altbox .summary",obj).css({
				'color':options.altjyFontColor
			});
			}
