...
if(!$operation) {
	...
	if(!submitcheck('submit')) {
		...
		//2020-02-15 添加后台插件控制检查更新
		loadcache('plugin');
		$open = isset($_G['cache']['plugin']['yunling_admin_update_plugin']['switch'])?$_G['cache']['plugin']['yunling_admin_update_plugin']['switch']:1;
		if ($open)
		{
		...
		foreach($plugins as $plugin) {
			...
			if($plugin['pluginid'] == $_GET['hl']) {
				...
				if($plugin['available'] && !$updateinfo) {
					if(empty($splitavailable[0])) {
						$title = '<tr><th colspan="15" class="partition">'.cplang('plugins_list_available').'（包括可更新插件）：'.$count_available.'</th></tr>';
						$splitavailable[0] = 1;
					}
				} elseif(!$plugin['available'] && !$updateinfo) {
					if(empty($splitavailable[1])) {
						$title = '<tr><th colspan="15" class="partition">'.cplang('plugins_list_unavailable').'：'.$count_unavailable.'</th></tr>';
						$splitavailable[1] = 1;
					}
				}
				elseif ($updateinfo)
                {
                    if (empty($splitavailable[2])) {
                        $title = '<tr><th colspan="15" class="partition">' . '可更新插件' .'：'. (is_array($checkresult)?count($checkresult):0) . '</th></tr>';
                        $splitavailable[2] = 1;
                    }
                }
			}
	...
	//2020-02-24 插件控制卸载插件后是否删除目录
	loadcache('plugin');
	if ($_G['cache']['plugin']['yunling_cloudaddons_uninstall']['uninstall_dir'])
