<?php echo 'Quater/cn.co anoucement';exit;?>
<!--{template common/header_ajax}-->
	<script type="text/javascript">
		...
		<!--{if $threadlist}-->
			...
			newthread[{$i}] = {'tid':$thread[tid], 'thread':{'icn':{'className':'icn','val':'<div class="forumava <!--{if $thread[folder] == 'lock' || $thread['special'] || $thread[displayorder]}-->s_lnoavatar<!--{/if}-->"><a href="home.php?mod=space&uid=$thread[authorid]" c="1"><!--{avatar($thread[authorid],small)}--></a></div>'},<!--{if $_G['forum']['ismoderator']}-->'o':{'className':'o','val':'<input type="checkbox" value="{$thread[tid]}" name="moderate[]" onclick="tmodclick(this)">'}<!--{/if}--> ,'forumtit':{'className':'new forumtit','val':'$thread[threadurl]<div class="foruminfo"><i class="y"><a href="forum.php?mod=viewthread&tid=$thread[tid]&{if $_GET['archiveid']}archiveid={$_GET['archiveid']}&{/if}extra=$extra" style="color: #EFAA00;" class="new_reply">有新的回复，马上点击查看！</a></i>{if !$_G['setting']['forumdisplaythreadpreview']}<a class="tdpre y" href="javascript:void(0);" onclick="javascript:previewThread(\'$thread[tid]\', \'$thread[id]\');">{lang preview}</a>{/if}
