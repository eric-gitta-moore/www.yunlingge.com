...
<!--{if count($comiis_app_nav['tnav'])}-->
	...
	<div class="comiis_top_sub bg_f">
		<div id="comiis_top_box"<!--{if $comiis_app_switch['comiis_subnv_top'] != 1}--> class="b_b"<!--{/if}-->>
			<div id="comiis_sub">
				<ul class="swiper-wrapper{if count($comiis_app_nav['tnav']) < 4} comiis_flex{/if}">
				<!--{loop $comiis_app_nav['tnav'] $temp}-->
					<li class="{if count($comiis_app_nav['tnav']) > 4}swiper-slide{else}flex{/if}{if $temp['nav_ids'] == $comiis_nav_ids} b_0{/if}"><a href="$temp['url']"{if $temp['nav_ids'] == $comiis_nav_ids} class="f_0"{else} class="f_c"{/if}>$temp['name']</a></li>
				<!--{/loop}-->
				</ul>
			</div>
		</div>
