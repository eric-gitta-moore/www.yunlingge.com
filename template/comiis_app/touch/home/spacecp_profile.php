		...
		<div id="comiis_sub">
			<ul class="swiper-wrapper">
				...
				<!--{if $_G[setting][verify]}-->
					<!--{loop $_G['setting']['verify'] $vids $verify}-->
						<!--{if $verify['available'] && (empty($verify['groupid']) || in_array($_G['groupid'], $verify['groupid']))}-->
							<!--{if $vids != 7}-->
							<li class="swiper-slide{if $opactives['verify'.$vids]} kmon{/if}"><a href="home.php?mod=spacecp&ac=profile&op=verify&vid=$vids"{if $opactives['verify'.$vids]} class="b_0 f_0"{else} class="f_c"{/if}>$verify['title']</a></li>
							<!--{/if}-->
						<!--{/if}-->
					<!--{/loop}-->
				<!--{/if}-->
