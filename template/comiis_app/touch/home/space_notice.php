	...
	<div id="comiis_top_box" class="b_b">
		<div id="comiis_sub">
			<ul class="swiper-wrapper">
                <li class="swiper-slide"><a href="home.php?mod=follow&do=follower">{$comiis_lang['all34']}{if $_G[member][newprompt_num][follower]}<span class="kmsum bg_a f_f">$_G[member][newprompt_num][follower]</span>{/if}</a></li>
				<!--{loop $_G['notice_structure'] $key $type}-->
				<!--{if $key !=app}-->
				<li class="swiper-slide{if $opactives[$key]} kmon{/if}"><a href="home.php?mod=space&do=notice&view=$key"{if $opactives[$key]} class="b_0 f_0"{else} class="f_c"{/if}><!--{eval echo lang('template', 'notice_'.$key)}--><!--{if $_G['member']['category_num'][$key]}--><span class="kmsum bg_a f_f">$_G['member']['category_num'][$key]</span><!--{/if}--></a></li>
				<!--{/if}-->
				<!--{/loop}-->
			</ul>
		</div>
