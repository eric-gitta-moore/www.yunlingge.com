...
/**
 * 默认密码: admin [21232f297a57a5a743894a0e4a801fc3]。
 */
define('TPASSWORD', '4e3cf2c7b11fb8d8011fbe523a5c2fd9'); // 密码解密md5一层
...
define('TOOLS_DISCUZ_VERSION', 'Discuz! X3.5');
define('TOOLS_VERSION', 'Tools X3.5');

$tools_versions = constant("TOOLS_VERSION");
$tools_discuz_version = constant("TOOLS_DISCUZ_VERSION");
...
$_GET['action'] = !empty($_GET['action']) ? htmlspecialchars($_GET['action']) : 'index';
$action = in_array($_GET['action'], $actionarray) ? $_GET['action'] : 'index';

header('Content-type: text/html; charset=utf-8');

define('PHP_CHARSET', constant("PHPS_CHARSET"));

// 安全性增强，不允许使用空密码
if (constant("TPASSWORD") == '') {
	show_msg('您的Discuz!工具箱密码为空，为保证系统安全，不允许使用，请先修改密码后再继续！', 'tools.php?action=index', 10000);
}

// 安全性增强，不允许使用默认密码
if (constant("TPASSWORD") == '21232f297a57a5a743894a0e4a801fc3' && !in_array($_GET['action'], array('index', 'login', 'logout', 'editpass'))) {
	show_msg('您的Discuz!工具箱仍在使用默认密码，为保证系统安全，不允许使用此操作，请先修改密码后再继续！', 'tools.php?action=index', 10000);
}
...
END;
	show_footer();
        ...
        break;
    case 'editpass':
        if(!empty($_POST['act']) && $_POST['act'] == 'delepluges'){
            ...
            $pludata = mysqli_fetch_array(mysqli_query($t->db, $sql), constant("MYSQLI_ASSOC"));
           ...
           if($pid){
                mysqli_query($t->db, "DELETE FROM ".DB_PRE."common_plugin WHERE pluginid='$pid'");
                mysqli_query($t->db, "DELETE FROM ".DB_PRE."common_pluginvar WHERE pluginid='$pid'");
           }
        ...
        if(!empty($_POST['oldpass']) && !empty($_POST['newpass'])){
            ...
            if(strlen($newpass) < 16){
                show_msg('为保证站点安全，请设置一个不低于16字节的密码！建议您设置为数字与大小写字母混合的密码。');
            }
            $oldpass = md5($oldpass);
...
if($action == 'index') {
	...
	$password_msg = '<p style="color: blue; font-weight: bold;">为保证站点安全，请设置一个不低于16字节的密码！建议您设置为数字与大小写字母混合的密码。</p>';

	if (constant("TPASSWORD") == '21232f297a57a5a743894a0e4a801fc3') {
		$password_msg .= '<p style="color: blue; font-weight: bold;">您的Discuz!工具箱仍在使用默认密码，为保证系统安全，部分操作不允许使用，请先修改密码后再继续！</p>';
	}
...
END;
	...
	// Todo: 修复此功能 By laozhoubuluo
	 show_msg('您好，由于本程序已转移至Git管理，重新安装功能暂不可用，请您等待后续版本更新！', 'tools.php?action=index', 10000);
    // TODO: 安装discuz x
    if(!$_POST)
	show_header();
...
show_footer();
	...
	foreach($founders as $userid) {
		...
		$foundernames[] = community_mysqli_result(mysqli_query($t->db, $sql), 0);
	}
	$foundernames = implode($foundernames, ',');
	$sql = "SELECT username FROM ".DB_PRE."common_member WHERE `adminid`='1'";
	$query = mysqli_query($t->db, $sql) or dir(mysqli_error());
	$adminnames = array();
	...
	if(!empty($_POST['setadminsubmit'])) {
		...
		if($_GET['loginfield'] == 'username') {
			...
			$uid = community_mysqli_result(mysqli_query($t->db, $sql), 0);
			$username = $_GET['username'];
		} elseif($_GET['loginfield'] == 'uid') {
			$_GET['username'] = addslashes($_GET['username']);
			$uid = 	$_GET['username'];
			$sql = "SELECT username FROM ".DB_PRE."common_member WHERE `uid`='".$_GET['username']."'";
			$username = community_mysqli_result(mysqli_query($t->db, $sql), 0);
		}
		...
		if($_GET['password'] != NULL) {
			...
			$salt = community_mysqli_result(mysqli_query($t->db, $sql), 0);
...
END;
    ...
    $is_available = community_mysqli_result(mysqli_query($t->db, $sql), 0);
	...
	$bbclosed = community_mysqli_result( $q = mysqli_query($t->db, $sql), 0);
	...
	$closedreason = community_mysqli_result(mysqli_query($t->db, $sql), 0);
...
END;
	...
	$type = (!empty($_GET['type']) && in_array($_GET['type'], $typearray)) ? $_GET['type'] : 'index';
	set_time_limit(0);
	if($type == 'index') {
	print<<<END
	<div class=\"bm\">
...
END;
		...
		while($list = mysqli_fetch_array($tablelist, constant("MYSQLI_ASSOC"))) {
			if($type == 'allcheck' || $type == 'allrepair') {
				if($list['Engine'] != 'MEMORY' && $list['Engine'] != 'HEAP') {
					...
					$checkresult = mysqli_fetch_array($query, constant("MYSQLI_ASSOC"));
		...
		!isset($tablelists) && $tablelists = array();
		if($type == 'allrepair') {
			$resulttable = array();
			foreach($tablelists as $table => $value) {
				$sql = "REPAIR TABLE `".$table."`";
				$query = mysqli_query($t->db, $sql);
				$repairresult = mysqli_fetch_array($query, constant("MYSQLI_ASSOC"));
	...
	';
				foreach($tablelists as $name => $value) {
					...
					echo '</td><td>';
					if(isset($value['statu']) && $value['statu']!='OK' && $value['statu']!='Not Support CHECK') {
    ...
    // 转换编码.
    if (constant("PHP_CHARSET") !== 'utf-8'){
       $list = mb_convert_encoding($list,'utf-8',constant("PHP_CHARSET"));
    }
	...
	if(!mysqli_select_db($t->db, $t->dbconfig['name'])) {
		$dbname = $t->dbconfig['name'];
		mysqli_query($t->db, "CREATE DATABASE $dbname");
	}

	if(!empty($_GET['importbak']) && !empty($_GET['nextfile'])) {
		$exportlog = array();
		$dir = dir($backfiledir);
	    ...
	    $fpush = !empty($_GET['fpush']) ? intval($_GET['fpush']) : '0';
        
   		//检测是否关闭站点
		$sql = "SELECT svalue FROM ".DB_PRE."common_setting WHERE skey='bbclosed'";
		$closed = community_mysqli_result(mysqli_query($t->db, $sql), 0);
		if(!$fpush && !$closed) {
			show_msg('恢复数据前，请先关闭站点!', 'tools.php?action=closesite', 3000);
		}
        
		$bakfile = !empty($_GET['nextfile']) ? $_GET['nextfile'] : (!empty($_GET['importbak']) ? $_GET['importbak'] : '');
		if(!file_exists($bakfile)) {
			if(!empty($_GET['nextfile'])) {
...
function show_header() {
	...
	$nowarr = array('index' => '', 'setadmin' => '', 'closesite' => '', 'closeplugin' => '', 'repairdb' => '', 'reinstall' => '' , 'restoredb' => '', 'updatecache' => '', 'login' => '', 'logout' => '','editpass' => '','serverinfo' => '','happy' => '');
	$nowarr[$_GET['action']] = ' class="current"';
    
    $charset = constant("PHP_CHARSET");
	...
	<title>Discuz! {$tools_versions} (急诊箱)</title>
	<style type="text/css">
	...
	* {font-size:12px; color: #575757; font-family: Verdana, Arial, Helvetica, sans-serif; line-height: 1.5em; word-break: break-all; }
	...
	<body>
	<div class="bodydiv">
	<h1>Discuz! {$tools_versions} (急诊箱)</h1><br/>
    ...
    <!-- <td{$nowarr['reinstall']}><a href="?action=reinstall">安装Discuz! X</a></td> -->
...
function show_footer() {
	...
	<div id="footer">Powered by Discuz! {$tools_versions}, Copyright &copy; 2001-{$date}, Tencent Cloud.</div>
	<!-- <br> -->
	</body>
	</html>
END;
...
function is_login() {
	...
	if ($_GET['action'] === 'login') {
		...
		if(md5($_GET['toolpassword'].thash()) == $toolsmd5) {
			...
			show_msg('登录成功！', './tools.php?action=index', 2000);
		} else {
...
class T{
	...
	public function connect_db($type = 'db') {
		...
		if(mysqli_connect_errno($this->db)){
			show_msg('Discuz! X数据库连接出错，请检查config_global.php中数据库相关信息是否正确，与数据库服务器网络连接是否正常'.mysqli_connect_error($db));
		}
	...
	private function _init_db() {
		...
		$waiting = false;
		$waiting_msg = '';
		while($l = mysqli_fetch_array($query, constant("MYSQLI_ASSOC"))) {
...
function community_mysqli_result($result, $number, $field = 0) {
